...
describe("apps SDK", () => {
  ...
  test("exposes theme management requests and change events", async () => {
    ...
    const state: ThemeEditorState = { selectedThemeId: "hiraya-dusk", themes: [{ id: "hiraya-dusk", name: "Hiraya Dusk", definition: { colors: Object.fromEntries(["shell", "chrome", "chromeText", "window", "windowMuted", "text", "textMuted", "accent", "accentText", "border", "danger", "dangerSurface", "desktopText", "selection"].map((key) => [key, "#000000"])) as ThemeDefinition["colors"], shape: { radius: 0, borderWidth: 0 }, effects: { blur: 0, opacity: 1, shadow: 0 }, typography: { family: "system", scale: 1, weight: 400 }, density: 1, motion: 1, iconSize: 48 }, builtIn: true, hasWallpaper: true }], canManage: true, restrictionReason: "" };

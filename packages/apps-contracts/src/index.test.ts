...
describe("apps contracts", () => {
  ...
  test("strictly validates the wallpaper editor protocol", () => {
    ...
    const state = { wallpaper, currentName: "Sky.webp", canManage: true, restrictionReason: "" };
    ...
    expect(parseRpcRequest({ protocolVersion: 1, type: "request", id: "select", method: "wallpapers.select", params: { handle: "file_0123456789abcdef" } })).toEqual(expect.objectContaining({ params: { handle: "file_0123456789abcdef" } }));
    expect(() => parseRpcRequest({ protocolVersion: 1, type: "request", id: "select", method: "wallpapers.select", params: { handle: "wallpaper-1" } })).toThrow("File handle");
    ...
    expect(() => parseWallpaperEditorState({ ...state, images: [] })).toThrow("unsupported shape");
  });
});
